...
export default function ChatPage() {
  ...
  const isDocumentMsg = (content: string) =>
    content.includes('SEÑOR JUEZ') ||
    content.includes('POR TANTO:') ||
    content.includes('PETITORIO') ||
    content.includes('---DOCUMENTO_COMPLETO---');
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#ffffff' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '24px 32px', maxWidth: '760px', width: '100%', margin: '0 auto', boxSizing: 'border-box' }}>
        {/* Capability pills — shown before first message */}
        {messages.length === 0 && (
          ...
          <div key={i} style={{
            ...
            {m.role === 'assistant' ? (
              <div style={{
                ...
                <div style={{ fontFamily: 'DM Mono, monospace', fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.1em', color: '#1a3d2b', marginBottom: '8px' }}>
                  arthur.ia
                </div>
                <SimpleMarkdown text={m.content.replace('---DOCUMENTO_COMPLETO---', '').trim()} />
                {(m.isDocument || isDocumentMsg(m.content)) && (
                  <button
                    onClick={() => void downloadWordFromMessage(m.content)}
                    style={{
                      marginTop: '12px',
          ...
          <div style={{ display: 'flex', marginBottom: '16px' }}>
            <div style={{
              ...
              <div style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
                {[0, 1, 2].map(i => (
                  <div key={i} style={{
                    width: '6px', height: '6px', borderRadius: '50%', background: '#1a3d2b',
      ...
      <div style={{
        ...
        background: '#ffffff', borderTop: '1px solid rgba(15,15,15,0.08)',
        padding: '16px 32px',
      }}>
        <div style={{ maxWidth: '760px', margin: '0 auto', position: 'relative' }}>
          ...
          {mentionDropdown && (
            <div style={{
              ...
              background: 'white', border: '1px solid rgba(15,15,15,0.12)',
              ...
              {filteredCasos.length === 0 ? (
                ...
                >
                  <span style={{ fontFamily: 'Inter, sans-serif', fontSize: '13px', fontWeight: 600, color: '#0f0f0f' }}>
                    {c.alias || c.cliente || 'Sin alias'}
                  </span>
                  <span style={{ fontFamily: 'DM Mono, monospace', fontSize: '10px', color: '#9ca3af' }}>
                    {c.numero_expediente}
                  </span>
                </button>
              ))}
          ...
          {selectedCaso && (
            <div style={{ marginBottom: '8px', display: 'flex', gap: '8px', alignItems: 'center' }}>
              <span style={{
                ...
                background: 'rgba(26,61,43,0.1)', color: '#1a3d2b',
                padding: '4px 10px',
                ...
                <button
                  onClick={removeSelectedCaso}
                  style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#1a3d2b', padding: '0', lineHeight: 1, fontSize: '14px' }}
                >
                  ×
                </button>
              </span>
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            <textarea
              ...
              rows={2}
              style={{
                flex: 1, border: '1px solid rgba(15,15,15,0.15)', padding: '12px 14px',
                fontFamily: 'Inter, sans-serif', fontSize: '14px', resize: 'none',
            ...
            <button
              ...
              style={{
                background: '#1a3d2b', color: 'white', border: 'none',
                padding: '12px 24px',
